...
import { ApiError, api, type BackendOfficeAsset, type OfficeAssetInput } from "@/lib/api";
import { formatCurrency, formatDate } from "@/lib/format";
import { toast } from "@/hooks/use-toast";

const blank: OfficeAssetInput = { assetTag: "", name: "", category: "", serialNumber: "", purchaseDate: "", purchaseCost: 0, assignedTo: "", notes: "" };
...
export default function OfficeAssets() {
  ...
  const load = useCallback(async () => {
    ...
    catch (error) { toast({ title: "Unable to load office assets", description: error instanceof ApiError ? error.message : "Request failed", variant: "destructive" }); }
  ...
  const save = async () => {
    setSaving(true);
    try {
      await api.createOfficeAsset({ ...form, purchaseDate: form.purchaseDate || null, serialNumber: form.serialNumber || null, assignedTo: form.assignedTo || null, notes: form.notes || null });
      setOpen(false); setForm(blank); await load(); toast({ title: "Office asset created" });
    } catch (error) { toast({ title: "Save failed", description: error instanceof ApiError ? error.message : "Request failed", variant: "destructive" }); }
  ...
  const columns: Column<BackendOfficeAsset>[] = [
    ...
    { key: "assignedTo", header: "Assigned to", render: (asset) => <span>{asset.assignedTo || "Unassigned"}</span> },
  ...
  return <div className="space-y-6">
    <PageHeader title="Office Assets" description="Internal equipment, assignments and acquisition details." actions={<Button variant="brand" onClick={() => setOpen(true)}><Plus className="mr-1 h-4 w-4" /> Add asset</Button>} />
    {loading ? <div className="flex justify-center gap-2 py-16 text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin" /> Loading assets…</div> : <DataTable data={assets} columns={columns} searchKeys={["assetTag", "name", "category", "serialNumber", "assignedTo"]} searchPlaceholder="Search assets…" emptyMessage="No office assets recorded." />}
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>Add office asset</DialogTitle></DialogHeader><div className="grid gap-3 py-2">
      ...
      <div className="grid grid-cols-2 gap-3"><Field label="Serial number"><Input value={form.serialNumber ?? ""} onChange={(e) => setForm({ ...form, serialNumber: e.target.value })} /></Field><Field label="Assigned to"><Input value={form.assignedTo ?? ""} onChange={(e) => setForm({ ...form, assignedTo: e.target.value })} /></Field></div>
